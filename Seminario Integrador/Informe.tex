...
Desde la perspectiva tecnológica, el sistema implementa una arquitectura en capas de cinco niveles claramente diferenciados: Core (modelos de dominio), Infraestructura (repositorios), Aplicación (servicios), Web (controladores REST) y Frontend (interfaz de usuario). El backend utiliza ASP.NET Core con Entity Framework sobre SQL Server, mientras que el frontend emplea React con TypeScript, integrándose mediante API REST con autenticación JWT y comunicación CORS configurada.
...
Está diseñado para atender tres perfiles diferenciados:
  \begin{itemize}
...
Los requerimientos funcionales se presentan clasificados en dos categorías según su estado de implementación: \textbf{Implementados} (funcionalidades CRUD core completadas) y \textbf{En Desarrollo} (consultas analíticas y reportes avanzados).
...
\hline
RF-03 & Gestión de Técnicas Malditas & Catálogo de técnicas malditas con su respectiva información. Registro de dominio de técnicas por hechicero.\\
\hline
...
\begin{itemize}
	\item Arquitectura en capas de 5 niveles con separación de responsabilidades (ver Sección 6 para descripción detallada)
	\item Inyección de dependencias nativa de ASP.NET Core con registro de servicios como Scoped
	\item Patrones Repository y Service Layer para abstracción de persistencia y lógica de negocio
\end{itemize}
...
\item \textbf{RNF-D05 - Testing}:
  \begin{itemize}
  \item Infraestructura de testing frontend configurada con Vitest + React Testing Library + jsdom 27.1.0, con implementación pendiente.
...
\begin{figure}[H]
	...
	\includegraphics[height = 9cm, width=\textwidth]{use}
	% --- Leyenda general de la figura ---
...
\begin{itemize}[left=0pt]
\item \textbf{Administrador/Soporte}: Rol administrativo del sistema responsable de mantener la integridad operativa de la plataforma. Gestiona usuarios, asigna y supervisa misiones críticas, genera reportes analíticos ejecutivos, administra el catálogo de hechiceros (altas, bajas, reasignaciones de grado), y configura parámetros globales del sistema. Tiene acceso completo a todas las funcionalidades CRUD y capacidad de auditoría sobre operaciones realizadas por otros roles. Actúa como supervisor estratégico de operaciones.

\item \textbf{Hechicero}: Actor operativo que ejecuta misiones de exorcismo. Consulta misiones asignadas con detalles de ubicación y maldición objetivo, actualiza el estado de su participación en misiones activas, registra técnicas malditas aplicadas durante combates con nivel de efectividad, reporta eventos ocurridos y daños colaterales, y visualiza su historial de misiones con estadísticas de desempeño. Puede actualizar información de su perfil profesional (técnicas dominadas, estado operativo).

\item \textbf{Personal de Apoyo}: Especialista en logística y coordinación operativa. Registra y gestiona traslados de hechiceros entre ubicaciones con seguimiento en tiempo real, administra inventario de recursos materiales y documenta su uso en misiones específicas, actualiza estado de disponibilidad de recursos y personal, coordina llegadas y salidas durante operaciones, y marca formalmente el fin de misiones para habilitar reportes post-operativos.

\item \textbf{Sistema}: Actor automático que ejecuta procesos sin intervención humana. Genera solicitudes de misión automáticamente cuando se detecta una nueva maldición activa, envía notificaciones a hechiceros sobre asignaciones y cambios de estado, calcula métricas de efectividad y estadísticas de desempeño, valida restricciones de negocio (compatibilidad de grados, disponibilidad de hechiceros), y mantiene logs de auditoría con timestamp para trazabilidad completa de operaciones críticas.
\end{itemize}
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.4\textwidth}
		...
		\caption{Creación de nuevas entidades}
		\label{fig:create}
	\end{subfigure}
	
	% --- Leyenda general de la figura ---
	\caption{Prototipos de interfaz interactiva}
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{\textwidth}
		...
		\caption{Vista de entidades existentes con opciones}
		\label{fig:view}
	\end{subfigure}
	% --- Subfigura (b) ---
	\begin{subfigure}[b]{\textwidth}
		\centering
		\includegraphics[width=\textwidth]{recent.png}
		\caption{Panel de acciones recientes}
		\label{fig:recent}
	\end{subfigure}
	
	% --- Leyenda general de la figura ---
	\caption{Prototipos de interfaz visual}
...
La metodología híbrida adoptada combina prácticas ágiles con planificación iterativa para adaptarse al contexto académico y al tamaño reducido del equipo. Los sprints de dos semanas se alinearon con la disponibilidad del equipo y el calendario de evaluaciones, mientras que la división en subequipos (backend/frontend), sustentada por contratos REST bien definidos, permitió la paralelización efectiva del trabajo. La gestión de riesgos mediante desarrollo incremental y validación temprana de decisiones arquitectónicas redujo trabajos repetidos costosos, promoviendo la calidad incremental y manteniendo la deuda técnica bajo control.
...
\section{Patrones de Diseño, Visualización y Datos \cite{fowler2002}}
...
\begin{itemize}[left=0pt]
	...
	\item \textbf{Container/Presenter (Smart/Dumb Components)}: Separación entre componentes y contenedores que gestionan el estado y la lógica (ej: \texttt{MissionsPage} con \texttt{useQuery}) y componentes de presentación puros que solo renderizan (ej: \texttt{MissionList} que recibe array de misiones y callbacks).
